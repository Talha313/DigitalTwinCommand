...
import { callHistory, getHistoryCall } from "@/lib/mock-data/call-history";
import type { CallStatus } from "@/lib/mock-data/types";

function matchesQuery(
  call: (typeof callHistory)[number],
  query: string,
): boolean {
  const q = query.trim().toLowerCase();
  if (!q) return true;
  if (call.caller.toLowerCase().includes(q)) return true;
  if (call.fromNumber.toLowerCase().includes(q)) return true;
  if (call.toNumber.toLowerCase().includes(q)) return true;
  if (call.summary.toLowerCase().includes(q)) return true;
  return call.transcript.some((u) => u.text.toLowerCase().includes(q));
}

export function CallHistoryClient() {
  const [query, setQuery] = React.useState("");
  const [status, setStatus] = React.useState<CallStatus[]>([]);
  ...
  const filtered = callHistory.filter((call) => {
    ...
    if (status.length > 0 && !status.includes(call.status)) return false;
    if (direction !== "all" && call.direction !== direction) return false;
    if (roleIds.length > 0 && !roleIds.some((id) => call.roleIds.includes(id))) {
      return false;
    }
    return true;
  });
  ...
  const selectedCall = selectedId ? getHistoryCall(selectedId) ?? null : null;
    ...
    <PageContainer>
      <div className="flex flex-col gap-1">
        ...
        <p className="text-sm text-muted-foreground">
          Review completed calls — transcripts, operator whispers, outcomes, and
          metadata retained for training.
        </p>
      </div>
      ...
      <div className="flex flex-col gap-3">
        ...
        <p className="text-xs text-muted-foreground">
          {filtered.length} of {callHistory.length} calls
        </p>
      </div>
